...
test('attack captures when steamrolling; survivors occupy, source loses the committed force', () => {
  ...
  assert.equal(r.state.territories.alaska.armies, 5); // 10 - 5 committed
  assert.equal(r.state.territories.nwt.armies, 5); // all survivors occupy
...
test('attack repulsed: 1 survivor retreats to source, ownership unchanged', () => {
  ...
  assert.equal(r.error, undefined);
  assert.equal(r.state.territories.nwt.owner, 1);
  assert.equal(r.state.territories.alaska.armies, 9); // 10 - 2 committed + 1 retreated
  assert.equal(r.state.lastCombat.captured, false);
});

test('capturing the opponent\'s last territory wins the game', () => {
  const s = attackState();
  s.territories.alberta.owner = 0; // now player 1 only owns nwt
  let rc = 0;
  const r = applyRiskAction({
...
test('illegal attack is rejected (target owned by self)', () => {
  const s = attackState();
  const r = applyRiskAction({
