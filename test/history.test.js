...
function setupDb() {
  ...
  insertGame(db, { id: 1, players: [1, 2], gameType: 'words' });
  return db;
}

test('appendTurnEntry assigns turn_number 1 for the first row', () => {
  const db = setupDb();
  const row = appendTurnEntry(db, 1, 0, 'play', { kind: 'play', words: ['HI'], scoreDelta: 5 });
...
test('appendTurnEntry increments turn_number per game', () => {
  const db = setupDb();
  appendTurnEntry(db, 1, 0, 'play', { kind: 'play', words: ['HI'], scoreDelta: 5 });
  const second = appendTurnEntry(db, 1, 1, 'pass', { kind: 'pass' });
  assert.equal(second.turnNumber, 2);
});

test('listTurnEntries returns rows oldest-first', () => {
  const db = setupDb();
  appendTurnEntry(db, 1, 0, 'play', { kind: 'play', words: ['ONE'], scoreDelta: 3 });
  appendTurnEntry(db, 1, 1, 'pass', { kind: 'pass' });
  appendTurnEntry(db, 1, 0, 'play', { kind: 'play', words: ['TWO'], scoreDelta: 4 });
...
test('appendTurnEntry isolates turn_number across games', () => {
  const db = setupDb();
  const now = Date.now();
  insertGame(db, { id: 2, players: [1, 2], gameType: 'rummikub' });
  appendTurnEntry(db, 1, 0, 'play', { kind: 'play', words: ['A'], scoreDelta: 1 });
  appendTurnEntry(db, 1, 1, 'pass', { kind: 'pass' });
  const first2 = appendTurnEntry(db, 2, 0, 'draw-tile', { kind: 'draw-tile' });
