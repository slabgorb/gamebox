...
// Pure builder for the Risk antique-map SVG/HTML. Verbatim port of the
// renderBoard body from plugins/risk/client/board.js — returns the full
// frame.innerHTML string so the React shell can hand it to
...
function armyToken(
  ...
  if (stack >= 3)
    g += `<g transform="translate(-6,-6)"><circle r="${r}" fill="${fill}" stroke="${ink}" stroke-width="1.6" opacity="0.85"/></g>`;
  if (stack >= 2)
    g += `<g transform="translate(-3.5,-3.5)"><circle r="${r}" fill="${fill}" stroke="${ink}" stroke-width="1.6" opacity="0.92"/></g>`;
  ...
  if (sel)
    g += `<circle r="${r + 5}" fill="none" stroke="var(--brass-2)" stroke-width="3" opacity="0.95"><animate attributeName="r" values="${r + 4};${r + 7};${r + 4}" dur="1.4s" repeatCount="indefinite"/></circle>`;
  return `${g}</g>`;
}

export function buildBoardHTML(view: RiskView, opts: BoardOpts): string {
  const { selected, plan = {}, to = null } = opts;
  const T = TERRITORIES as any;
  const C = CONTINENTS_META as any;
  const { w: MW, h: MH } = MAP_SIZE as any;
  ...
  let washes = "";
  for (const [id, g] of Object.entries(T) as [string, any][]) {
  ...
  let banners = "";
  for (const c of Object.values(C) as any[]) {
  ...
  // ---- italic sea labels ----
  const seaLabels = (SEA_LABELS as any[])
    .map(
      (l) =>
  ...
  for (const [id, g] of Object.entries(T) as [string, any][]) {
    ...
    hits += `<g class="${cls}">`;
    if (isSel)
      hits += `<path d="${g.path}" fill="#fbe79a" opacity="0.22" pointer-events="none"/>`;
    if (isTarget)
      hits += `<path d="${g.path}" fill="#a13b2a" opacity="0.14" pointer-events="none"/>`;
  ...
  let seams = "";
  for (const seam of LAND_SEAMS as any[]) {
    ...
    const d = seam.path
      .map((p: number[], j: number) => `${j === 0 ? "M" : "L"} ${p[0]} ${p[1]}`)
      .join(" ");
    seams += "<g>";
    seams += `<path d="${d}" fill="none" stroke="#fbf2d6" stroke-width="${sw + 2.4}" opacity="0.6" stroke-linecap="round" stroke-linejoin="round"/>`;
    if (involved)
      seams += `<path d="${d}" fill="none" stroke="#fbe79a" stroke-width="${sw + 5}" opacity="0.6" stroke-linecap="round" stroke-linejoin="round"/>`;
  ...
  if (sel) {
    for (const n of sel.neighbors) {
      ...
      arcs +=
        `<g><path d="${d}" stroke="rgba(251,232,160,0.85)" stroke-width="7.6" fill="none" stroke-linecap="round"/>` +
  ...
  // ---- placename labels ----
  let labels = "";
  for (const [, g] of Object.entries(T) as [string, any][]) {
  ...
  return (
    '<span class="map-corner-screw tl"></span><span class="map-corner-screw tr"></span>' +
