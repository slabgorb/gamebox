...
**Addendum (2026-05-19):** Task 5.4 Step 4 ("manual parity") missed three DiceTray bugs in the shared layer: dice spawned embedded in the ceiling collider (clip-through), the `<dice-tray>` custom element rendered as `display: inline` (collapsed layout), and `roll(N)` applied one ThrowParams to every die (always-doubles). The "open risk" flagged above (`<dice-tray>.throw()` granularity) is exactly what bit us — the documented one-call-per-die loop was wrong, the bundle wants a single per-die-array batch. Fixed in followup plan `2026-05-19-cycle1-dicetray-parity-fixes.md`. Future cycles' manual parity steps should explicitly walk the dice path (roll variety, frame containment) whenever DiceTray is in use.
